...
functions.autolabel(ax, protoss_bar, [str(value)[:4] + '%' if (value or value == 0) else '?' for value in protoss_data])
functions.autolabel(ax, terran_bar, [str(value)[:4] + '%' if (value or value == 0) else '?' for value in terran_data])
functions.autolabel(ax, zerg_bar, [str(value)[:4] + '%' if (value or value == 0) else '?' for value in zerg_data])
